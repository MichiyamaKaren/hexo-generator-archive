...
const archiveByDate = function(locals) {
  const { config } = this;
...
const archiveByChapter = function(locals) {
  const { config } = this;
  ...
  allPosts.forEach(post => {
    ...
    if (!Object.prototype.hasOwnProperty.call(posts, chapter)) {
      posts[chapter] = [];
    }

    posts[chapter].push(post);
  });

  const { Query } = this.model('Post');
  const chapters = Object.keys(posts);
  let chapter, data, url;

  for (let i = 0, len = chapters.length; i < len; i++) {
    chapter = chapters[i];
    data = posts[chapter];
    url = archiveDir + chapter + '/';
    if (!data.length) continue;

    generate(url, new Query(data), { chapter });
  }
...
module.exports = function(locals) {
  const { config } = this;
  if (config.archive_generator.by_chapter) {
    return archiveByChapter.call(this, locals);
  } else {
    return archiveByDate.call(this, locals);
  }
}